...
{	short prev;
	...
	if( !processed_done ) 
	{
	    ...
	    while( (processedSize < total_length) && run ){
			// printf("processedSize %lu / total_length %lu\n", processedSize, total_length);
			if(listen) 
			{
				if(!prev)
				{
...
void st_MP3_Open(){
    ...
    mp3dec_ex_open(&mp3d, soundfile, MP3D_SEEK_TO_SAMPLE);

// mp3d.info.hz = 49170;

	long this_samplerate = mp3d.info.hz;
	int this_channels = mp3d.info.channels;

	off_t length = mp3d.detected_samples >> 1;
	// printf("detected_samples %llu\n", length);
...
void st_MP3_Decode( int8_t* this_pBuffer ){
	...
	u_int32_t done = 0;
	size_t current_done = 1;

    while( done < bufferSize && current_done) {

		size_t packet_size = MINIMP3_MAX_SAMPLES_PER_FRAME * sizeof(mp3d_sample_t);
		unsigned char* pData = new unsigned char[packet_size];

		current_done = mp3dec_ex_read(&mp3d, (mp3d_sample_t*)pData, packet_size / sizeof(mp3d_sample_t));

		current_done *= sizeof(mp3d_sample_t);
